...
namespace Arcanoid
{
    public partial class FormMainGameWindow : Form
    {
        ...
        // добавлением линии новых обьектов
        public void AddLine()
        {
        ...
        public bool IsCollide()
        {
            ...
            if (map[ballY + dirY, ballX] != 0)
            {
                ...
                if (addScore)
                    // Усложняем игру добавлением новых блоков
                {
                    score += 50;
                    ...
                    if  (score % 5 == 0 && score >0 && timer1.Interval > 0)
                    {
        ...
        public void Init()
        {
            ...
            arcanoidSet = new Bitmap(@"D:\repos\Arcanoid\Arcanoid\img\arcanoid.png");
            // скорость
            timer1.Interval = 200;
            ...
            map[ballY, ballX] = 8;
            // Отражаем направление после столкновения
            dirX = 1;
            dirY = -1;
        ...
        public void DrawMap(Graphics g)
        {
            for (int i = 0; i < mapHeight; i++)
            {
                for (int j = 0; j < mapWidth; j++)
                {
                    ...
                    if (map[i, j] == 2)
                    {
                        g.DrawImage(arcanoidSet, new Rectangle(new Point(j * 20, i * 20), new Size(40, 20)), 20, 16 + 75 * (map[i, j] - 1), 170, 60, GraphicsUnit.Pixel);
                    }
                    if (map[i, j] == 3)
                    {
                        g.DrawImage(arcanoidSet, new Rectangle(new Point(j * 20, i * 20), new Size(40, 20)), 20, 16 + 75 * (map[i, j] - 1), 170, 60, GraphicsUnit.Pixel);
